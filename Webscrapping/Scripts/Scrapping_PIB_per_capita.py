...
def procesar_datos(nombre_archivo_csv):
    dataframe_completo = pd.read_csv(nombre_archivo_csv)
    dataframe_desde_2000 = dataframe_completo.iloc[0:dataframe_completo.shape[0]-2,[0] + list(range(21, dataframe_completo.shape[1]))]
    #print(dataframe_desde_2000)

if __name__ == '__main__':

    nombre_archivo_excel = "../DataSets/pib_per_capita.xls"
    nombre_archivo_csv = "../DataSets/pib_per_capita.csv"
    obtener_datos(nombre_archivo_excel, nombre_archivo_csv)
    procesar_datos(nombre_archivo_csv)
